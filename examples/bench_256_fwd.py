...
kernel differs:
  ...
  - FASTVIDEO_VSA_CUTEDSL=1  -> FA4 CuTe: (Q 256 x KV 128) tiles via
                               flash_attn.cute (forward only, Blackwell sm_100+).
...
def _time(thunk, iters, warmup):
    try:
        ...
        msg = str(exc).strip().splitlines()[-1][:60] if str(exc).strip() else ""
...
def run(name, nb, heads, dim, sparsity, iters=30, warmup=15):
    ...
    block_map = _topk_map(heads, nb, topk, "cuda")
    ...
    outs = {}
    for label, backend in [("route-A Triton 256->64", "triton"),
                           ("CuTe FA4 256x128", "cutedsl")]:
    ...
    if "triton" in outs and "cutedsl" in outs and outs["triton"] is not None \
            and outs["cutedsl"] is not None:
        d = (outs["triton"].float() - outs["cutedsl"].float()).abs()
        print(f"    max|triton-cute| = {d.max().item():.3e}   "
              f"mean = {d.mean().item():.3e}", flush=True)

    del q, k, v, block_map, vbs, outs
    torch.cuda.empty_cache()
